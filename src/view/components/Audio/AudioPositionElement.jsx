...
export function AudioPositionElement() {
	...
	if (typeof trackStatus.duration === "number") {
		...
		statusCompletedPercentage = parseFloat(statusCompletedPercentage.toFixed(4));
	...
	const handleHitbox = (value) => {
		if (!audioRef?.current || !audioRef?.current?.duration || (!value && value !== 0))
			return false;
